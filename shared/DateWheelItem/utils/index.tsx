...
function getDayStatus(
  ...
  if (isCheatDay) return "cheatMeal";
...
export function useDayPickerData(
  ...
  return useMemo(() => {
    ...
    for (let dateNumber = 1; dateNumber <= daysInMonth; dateNumber++) {
      ...
      const isPastFlag = dateNumber < todayDateNumber;
      const isFutureFlag = dateNumber > todayDateNumber;
      const isSelectedFlag =
        selectedDateIndex !== undefined && dateNumber === selectedDateIndex + 1;
      const isCheatDay = isCheatMealDate(date, cheatMealDay, periodOfDays);
      const totalCalories = dayData?.totalCalories || 0;
      const dayStatus =
        isPastFlag || isTodayFlag
          ? getDayStatus(totalCalories, dailyGoal, isCheatDay)
          : null;
